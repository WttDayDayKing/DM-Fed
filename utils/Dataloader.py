...
class DataLoader(Dataset):
    def __init__(self):
        ...
        datasets_dir = "/data/wtt/data_set/FL_CL/Matek-19"
        keys = np.unique([x for x in os.listdir(datasets_dir)])

        for k in keys:
            datasets[k] = [x for x in os.listdir(datasets_dir) if x == k]

        samples = {}
        for dataset in datasets:
            for file in datasets[dataset]:
                print("loading ", dataset, "... ", end="", flush=True)
                with open(os.path.join(datasets_dir, file), "rb") as f:
                    data = pickle.load(f)
                    for d in data:
