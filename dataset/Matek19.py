...
class matek19(Dataset):
    def __init__(self,args, is_train=True, index=None,
        ...
        if is_train:
            ...
            if base_session:
                ...
                self.data, self.targets = self.SelectfromClasses(self.data, self.targets, index, old_index,current_task)
        else:
            self.transform = transforms.Compose([
                transforms.Resize((224,224)),
                transforms.ToTensor(),
    ...
    def SelectfromClasses(self, data, targets, index, old_index=None,current_task=0):
        ...
        if (self.is_train==False and old_index is not None) or (self.args.use_old_class_index and old_index is not None):
            for i in old_index:
                ind_cl = np.where(i == targets)[0]
            ...
            # else:
            #    data_base.update_new_set(self,data_tmp,targets_tmp,index)
